  ...
  return (
    <nav
      ...
      <div className="dropdownn w-full lg:hidden xl:hidden md:hidden uppercase fixed top-0 left-0 z-10 "  >
        <div className={`flex justify-between gap-3 w-full pb-3 py-4 items-center px-5 ${scrollNavbar && darkMode  ? "bg-black text-white" : "" || scrollNavbar ? "bg-white" : ""}`} id="appNav">
          <div className={`uppercase font-bold w-[100%]
               ${darkMode ? "text-white" : ""}`
               } 
               onClick={tutup}>
            trajuszt
          ...
          <div className="flex gap-3">
            <i class={`fa-regular cursor-pointer ${darkMode  ? "fa-sun text-white" : "fa-moon"}`} onClick={toogleDarkMode}></i>
            <i class={`fa-solid fa-bars cursor-pointer ${darkMode ? "text-white" : ""}`} onClick={toggleDropdown}></i>  
          </div>
        </div>
